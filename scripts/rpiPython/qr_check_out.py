...
def draw_panel(
    ...
    cv2.putText(frame, "CHECK-OUT", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (220, 220, 220), 2, cv2.LINE_AA)
    ...
    if detail:
        cv2.putText(frame, detail[:18], (154, 206), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (90, 180, 255), 2, cv2.LINE_AA)
...
def main() -> None:
    ...
    last_scan_at = 0.0
    state = make_state("Ready for check-out")
    ...
    try:
        while True:
            ...
            cv2.putText(frame, "CHECK-OUT", (20, frame_h - 22), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (90, 180, 255), 2, cv2.LINE_AA)
            ...
            if value and now - last_scan_at >= SCAN_COOLDOWN_SECONDS:
                ...
                try:
                    ...
                    state["detail"] = f"{duration_minutes} min"
            ...
            if now > state["until"] and state["message"] != "Ready for check-out":
                state = make_state("Ready for check-out")
            ...
            cv2.imshow("Tap-it Check-Out Scanner", frame)
