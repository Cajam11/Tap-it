...
export default async function AdminUserProfilePage({
  ...
  if (!profile || profileError) {
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center">
        ...
        <Link href="/admin/users" className="inline-flex items-center gap-2 rounded-xl bg-white/5 px-4 py-2 text-sm text-white hover:bg-white/10 transition-colors">
  ...
  const activeMembership = (activeMemberships as ActiveMembership[] | null)?.[0] || null;
  ...
  const hasCurrentMembership = activeMembership && !isExpiredMembership(activeMembership);
  ...
  const getTransactionLabel = (tx: Transaction) => {
    const membership = Array.isArray(tx.memberships) ? tx.memberships[0] : tx.memberships;
    const membershipName = membership?.name || null;
    const metadata = getMetadataObject(tx.metadata);
    const action = metadata.action === 'cancel' ? 'cancel' : tx.type ?? "unknown";
  ...
  return (
    <section className="space-y-8 pb-20 max-w-6xl mx-auto relative">
      ...
      <div className="pt-2">
        <Link href="/admin/users" className="inline-flex items-center gap-2 text-sm text-white/50 hover:text-white transition-colors">
          <ArrowLeft className="h-4 w-4" />
      ...
      <article className="relative rounded-3xl border border-white/10 bg-white/[0.03] p-6 sm:p-8 backdrop-blur-xl mt-4">
        <div className="grid gap-8 lg:grid-cols-[220px_minmax(0,1fr)] lg:items-center">
          <div className="flex flex-col items-center gap-4 lg:items-start">
            ...
            <p className="text-xs text-white/40">Zaregistrovaný {formatDateShort(profile.created_at)}</p>
          ...
          <div>
             <p className="text-xs uppercase tracking-[0.2em] text-white/45">Profil Používateľa</p>
             <h1 className="mt-2 text-4xl font-black leading-tight tracking-tight text-white">{profile.full_name || "Neznámy Užívateľ"}</h1>
             <p className="mt-2 text-sm text-white/55">{profile.email || "Bez e-mailu"}</p>

             <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                <div className="rounded-2xl border border-white/10 bg-white/[0.02] px-4 py-3">
                  <p className="text-xs uppercase tracking-wide text-white/45">Onboarding</p>
                  <p className="mt-1 text-sm font-semibold text-white">{profile.onboarding_completed ? "Dokončený" : "Čakajúci"}</p>
                </div>
                <div className="rounded-2xl border border-white/10 bg-white/[0.02] px-4 py-3">
                  <p className="text-xs uppercase tracking-wide text-white/45">Členstvo</p>
                  <p className="mt-1 text-sm font-semibold text-white">{hasCurrentMembership ? membershipRecord?.name || "Aktívne" : "Neaktívne"}</p>
                </div>
                {(hasCurrentMembership && activeMembership?.start_date) ? (
                  <>
                    <div className="rounded-2xl border border-white/10 bg-white/[0.02] px-4 py-3">
                      <p className="text-xs uppercase tracking-wide text-white/45">Platí Od</p>
                      <p className="mt-1 text-sm font-semibold text-white">{formatDateShort(activeMembership.start_date)}</p>
                    </div>
                    <div className="rounded-2xl border border-white/10 bg-white/[0.02] px-4 py-3">
                      <p className="text-xs uppercase tracking-wide text-white/45">Platí Do</p>
                      <p className="mt-1 text-sm font-semibold text-white">{activeMembership?.end_date ? formatDateShort(activeMembership.end_date) : "Trvalé"}</p>
                    </div>
                  </>
                ) : (
                  <>
            ...
            <div className="mb-4 flex items-center justify-between text-white">
              ...
              {activeOpenEntry && (
                <form action={manualCheckOutUser}>
                  <input type="hidden" name="entryId" value={activeOpenEntry.id} />
                  <button type="submit" className="flex items-center gap-1.5 rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-1.5 text-xs font-semibold uppercase tracking-wide text-amber-300 hover:bg-amber-500/20 transition-colors">
              ...
              {entries && entries.length > 0 ? (
                <ul className="space-y-4">
                  {entries.map(entry => {
                    ...
                    return (
                      <li key={entry.id} className={`flex items-center justify-between rounded-xl border p-4 ${isOpen ? "border-amber-500/20 bg-amber-500/5 text-amber-50" : "border-white/10 bg-white/[0.02] text-white"}`}>
                        <div className="flex items-center gap-4">
                          <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full ${isOpen ? 'bg-amber-500/20 text-amber-400' : 'bg-white/5 text-white/50'}`}>
                            {isOpen ? <Activity className="h-4 w-4" /> : <ArrowRightLeft className="h-4 w-4" />}
                          </div>
                          <div>
                            <p className="text-sm font-semibold">
                              {formatDateShort(entry.check_in)}
                            </p>
                            <p className={`flex items-center gap-1.5 text-xs mt-0.5 ${isOpen ? "text-amber-200/70" : "text-white/55"}`}>
                              <span>IN: {formatTime(entry.check_in)}</span>
                              <span className="w-1 h-1 rounded-full bg-current opacity-30" />
                              <span>OUT: {entry.check_out ? formatTime(entry.check_out) : "Čakajúci"}</span>
                            </p>
                          </div>
                        </div>
                        <div>
              ...
              {transactions && transactions.length > 0 ? (
                <ul className="space-y-4">
                  {transactions.map(tx => {
                    ...
                    return (
                    <li key={tx.id} className="flex items-center justify-between rounded-xl border border-white/10 bg-white/[0.02] p-4 text-white">
                      <div className="flex items-center gap-4 flex-1">
                        ...
                        <div className="flex-1">
                          <p className="text-sm font-semibold">
                            {tx.amount} {tx.currency === "EUR" ? "€" : tx.currency}
