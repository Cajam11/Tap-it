...
export default function WeightChart({ logs, className }: WeightChartProps) {
  ...
  if (!isMounted) {
    return (
      <div
        className={cn(
          "flex h-full min-h-[16rem] w-full animate-pulse flex-col items-start justify-end gap-6 rounded-2xl border border-white/5 bg-white/[0.02] p-6",
          className,
        )}
      >
  ...
  if (!chartData || chartData.length === 0) {
    return (
      <div
        className={cn(
          "flex h-full min-h-[16rem] w-full flex-col items-center justify-center rounded-2xl border border-white/10 bg-white/5 p-4 text-center text-sm text-white/40",
          className,
        )}
      >
  ...
  let Y_DOMAIN: [number, number] = [
    Math.floor(minWeight - 5),
  ...
  if (chartData.length === 1) {
    Y_DOMAIN = [
      Math.floor(chartData[0].weight_kg - 5),
      Math.ceil(chartData[0].weight_kg + 5),
    ];
  }
        ...
        >
          ...
          <Tooltip
            ...
            formatter={(value: unknown) => [
              `${typeof value === "number" ? value : Number(value ?? 0)} kg`,
              "Váha",
            ]}
            labelStyle={{
              color: "#ffffff80",
